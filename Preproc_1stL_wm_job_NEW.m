...
% ALL-SUBJECTS SCRIPT TO PROCESS AND RUN 1ST-LEVEL ANALYSIS OF WORKING MEMORY TASK (WM)
clear all;
close all;
...
for i = 1:length(subjects_list)
    ...
    epi_file = fullfile(analysis_dir,subjects_list{i},'func', [subjects_ids{i},'.EPI_fMRI.WM.PA.mcf.topped.nii']);
...
for i = 1:length(subjects_list)
    ...
    % dataset = [analysis_dir subjects_list{i} '/func/' 'sw' subjects_ids{i} '.EPI_fMRI.WM.PA.mcf.topped.nii']; % load only preprocessed data
    % ---NOTE: warped but unsmoothed images for future graph theory analysis...
    dataset = [analysis_dir subjects_list{i} '/func/' 'w' subjects_ids{i} '.EPI_fMRI.WM.PA.mcf.topped.nii']; % load only preprocessed data
    volumes = spm_select('Expand',dataset);

    % Extract the task and scan data (including 6 movement params) - LOADS SUBJECT STRUCTURE!
    structure = fullfile(behavioral_dir,subjects_list{i},'subjectinfo_EPI_fMRI_WM_PA_mcf_topped_nii.mat');
    load(structure);
    new_par_filepath = fullfile(par_files_dir,subjects_list{i}, [subjects_ids{i},'.EPI_fMRI.WM.PA.mcf.par']);
    ...
    if ~isempty(subject.scan_trimmed_length) && (i==1 || i==3 || i==7 || i==13 || i== 20) % (..pseudonyms..)
    ...
    aCompCor_file = fullfile(par_files_dir,subjects_list{i},'WM_acompcor_all.tsv');
    if ~exist(aCompCor_file)
        error('aCompCor components not computed yet for WM task of %s', subject.id);
    end
    ...
    subject.raw_dataset = [analysis_dir subjects_list{i} '/func/' subjects_ids{i} '.EPI_fMRI.WM.PA.mcf.topped.nii'];
    ...
    if i==1 || i==3 || i==7 || i==13 || i== 20 % (..pseudonyms..)
        Trim = 1;
    else
        Trim = 0;
    end
    ...
    if Trim == 1
        firstLevel_dir = [analysis_dir subjects_list{i} '/func/' '1stLevel_wm_trimmed_optcens'];
        if ~exist(firstLevel_dir)
            mkdir(firstLevel_dir);
        end
    elseif Trim == 0
        firstLevel_dir = [analysis_dir subjects_list{i} '/func/' '1stLevel_wm_untrimmed_optcens'];
        if ~exist(firstLevel_dir)
            mkdir(firstLevel_dir);
        end
    end
    ...
    if Trim == 1
        ...
        elseif isempty(subject.scan_trimmed_length)
            fprintf(['WM scan was not trimmed for ', subjects_ids{i},'! \n']);
    ...
    % regressors_matrix_file = fullfile(out_dir, 'wm_all_regressors.tsv');
    regressors_matrix_file = fullfile(firstLevel_dir, 'wm_all_regressors.mat');
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {[firstLevel_dir '/wm_all_regressors.mat']};
