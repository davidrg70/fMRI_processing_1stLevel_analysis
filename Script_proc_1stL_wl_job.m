% MULTIPLE-SUBJECTS SCRIPT LANGUAGE TASK (WL) (2021) DG (& DvdV)
...
for i = 1:length(subjects_list)
    ...
    if ~exist([analysis_dir '/' subjects_list{i} '/func/' 'Preproc_wl'])
        mkdir([analysis_dir '/' subjects_list{i} '/func/' 'Preproc_wl']);
    end    
    
    % check the ART preprocessing in existing files:
    load(fullfile(analysis_dir, subjects_list{i}, ['/conf/', 'EPI_fMRI_language_PA_mcf_topped_nii', '/subjectinfo_EPI_fMRI_language_PA_mcf_topped_nii.mat']));
...
%     end
    ...
    matlabbatch{2}.cfg_basicio.file_dir.file_ops.cfg_named_file.files = {{[analysis_dir '/' subjects_list{i} '/func/' subjects_list{i} '.EPI_fMRI.language.PA.mcf.topped' '.nii']}};
    % matlabbatch{2}.cfg_basicio.file_dir.file_ops.cfg_named_file.files = {{[analysis_dir '/' subjects_list{i} '/func/' 'art_' subjects_list{i} '.EPI_fMRI.language.PA.mcf.topped_' , ARTthreshold, '.nii']}};
    matlabbatch{3}.cfg_basicio.file_dir.dir_ops.cfg_named_dir.name = 'Basedir';
    matlabbatch{3}.cfg_basicio.file_dir.dir_ops.cfg_named_dir.dirs = {{[analysis_dir '/' subjects_list{i} '/func/' 'Preproc_wl']}};
...
for i = 1:length(subjects_list)
    ...
    structure = [analysis_dir '/' subjects_list{i} '/conf/' 'EPI_fMRI_language_PA_mcf_topped_nii/' 'subjectinfo_EPI_fMRI_language_PA_mcf_topped_nii.mat'];
    load(structure);
    [subject] = dg_child_taskdata_trim(subject); % BRINGS TASK DATA AND TRIMS DATASET (TRIMMING ALSO THE 6 MOVEMENT PARAMETERS)
    
    if ~exist([analysis_dir '/' subjects_list{i} '/func/' '1stLevel_wl'])
        mkdir([analysis_dir '/' subjects_list{i} '/func/' '1stLevel_wl']);
    end
    
    cd([analysis_dir '/' subjects_list{i} '/func/' '1stLevel_wl']);
    matlabbatch{1}.spm.stats.fmri_spec.dir = {[analysis_dir '/' subjects_list{i} '/func/' '1stLevel_wl']};
    matlabbatch{1}.spm.stats.fmri_spec.timing.units = 'secs';
    matlabbatch{1}.spm.stats.fmri_spec.timing.RT = 1.4;
    matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t = 16;
    matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t0 = 8;
    
    dataset = [analysis_dir '/' subjects_list{i} '/anat/' 'sw' subjects_list{i} '.EPI_fMRI.language.PA.mcf.topped.nii'];
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).name = 'Animals';
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).name = 'Fractals';
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {[analysis_dir '/' subjects_list{i} '/func/' 'mov_params_wl.mat']};
    ...
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.name = 'Animals > Fractals';
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
    matlabbatch{5}.spm.stats.con.consess{2}.tcon.name = 'Animals < Fractals';
