...
% ALL-SUBJECTS SCRIPT TO PROCESS AND RUN 1ST-LEVEL ANALYSIS OF LANGUAGE TASK (WL)
clear all; 
close all;
...
FD_threshold = 0.3; % 0.3mm used ORIGINALLY by mw_optcens.m script, lines 100 and 101!
...
for i = 1:length(subjects_list)
    ...
    epi_file = fullfile(analysis_dir,subjects_list{i},'func', [subjects_ids{i},'.EPI_fMRI.language.PA.mcf.topped.nii']);
...
for i = 1:length(subjects_list)
    ...
    % dataset = [analysis_dir subjects_list{i} '/func/' 'sw' subjects_ids{i} '.EPI_fMRI.language.PA.mcf.topped.nii']; % load only preprocessed data
    % ---NOTE: warped but unsmoothed images for future graph theory analysis...
    dataset = [analysis_dir subjects_list{i} '/func/' 'w' subjects_ids{i} '.EPI_fMRI.language.PA.mcf.topped.nii']; % load only preprocessed data
    volumes = spm_select('Expand',dataset);

    % Extract the task and scan data (including 6 movement params) - LOADS SUBJECT STRUCTURE!
    structure = fullfile(behavioral_dir,subjects_list{i},'subjectinfo_EPI_fMRI_language_PA_mcf_topped_nii.mat');
    load(structure);
    new_par_filepath = fullfile(par_files_dir,subjects_list{i}, [subjects_ids{i},'.EPI_fMRI.language.PA.mcf.par']);
    ...
    % trim the .txt file (.par copy) if the participant had data trimmed
    if ~isempty(subject.scan_trimmed_length) && (i==11 || i==19) % (pseudonym and pseudonym)
        M = readmatrix(rp_filename);
        M(length(subject.scan_trimmed_length)+1:end,:) = []; % cut rows n+1 to end
        writematrix(M, rp_filename, 'Delimiter','\t');
    end
    ...
    aCompCor_file = fullfile(par_files_dir,subjects_list{i},'language_acompcor_all.tsv');
    if ~exist(aCompCor_file)
        error('aCompCor components not computed yet for Language task of %s', subject.id);
    end
    ...
    if mean_FD >= FD_threshold
        warning('%s Mean FD = %.2f, greater than/equal to FD threshold (%.1f) \n', subjects_ids{i}, mean_FD, FD_threshold);
    ...
    if scrubbing_proportion >= scrubbing_threshold
        % warning('%.2f%% of volumes will be scrubbed, greater than/equal to scrubbing threshold (%.0f%%) \n', scrubbing_percent, scrubbing_threshold_percent);
    else
    ...
    subject.raw_dataset = [analysis_dir subjects_list{i} '/func/' subjects_ids{i} '.EPI_fMRI.language.PA.mcf.topped.nii'];
    ...
    % only do trimming for certain subjects, only those with more volumes
    if i==11 || i==19 % (pseudonym and pseudonym)
        Trim = 1;
    else
        Trim = 0;
    end
    ...
    if Trim == 1
        firstLevel_dir = [analysis_dir subjects_list{i} '/func/' '1stLevel_wl_trimmed_optcens'];
        if ~exist(firstLevel_dir)
            mkdir(firstLevel_dir);
        end
    elseif Trim == 0
        firstLevel_dir = [analysis_dir subjects_list{i} '/func/' '1stLevel_wl_untrimmed_optcens'];
        if ~exist(firstLevel_dir)
            mkdir(firstLevel_dir);
        end
    end
    ...
    if Trim == 1
        ...
        elseif isempty(subject.scan_trimmed_length)
            fprintf(['Language scan was not trimmed for ', subjects_ids{i},'! \n']);
    ...
    % regressors_matrix_file = fullfile(out_dir, 'language_all_regressors.tsv');
    regressors_matrix_file = fullfile(firstLevel_dir, 'language_all_regressors.mat');
    ...
    % BRINGING RESULTS_Animals-Phonological
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).name = 'Phonological';
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).onset = [subject.WLtask.Onsets_animals];
    
    % BRINGING RESULTS_Duration_Animals-Phonological
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).duration = [subject.WLtask.RT_animals];
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).tmod = 0;
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).pmod = struct('name', {}, 'param', {}, 'poly', {});
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).orth = 1;
    
    % BRINGING RESULTS_Fractals-Visual
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).name = 'Visual_Fractals';
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).onset = [subject.WLtask.Onsets_fractals];
    
    % BRINGING RESULTS_Duration_Fractals-Visual
    matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).duration = [subject.WLtask.RT_fractals];
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {[firstLevel_dir '/language_all_regressors.mat']};
    ...
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.name = 'Phonological > Visual Fractals';
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
    matlabbatch{5}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
    matlabbatch{5}.spm.stats.con.consess{2}.tcon.name = 'Phonological < Visual Fractals';
